...
class GlazeWMClient:
    def __init__(
            self,
            glazewm_path: str = "glazewm.exe",
            timeout_secs: float = 0.5
    ):
        super().__init__()
        self._timeout_secs = timeout_secs
        self._glazewm_path = glazewm_path
    ...
    def query_state(self) -> Optional[dict]:
        with suppress(json.JSONDecodeError, subprocess.CalledProcessError, subprocess.TimeoutExpired):
            output = subprocess.check_output([self._glazewm_path, "state"], timeout=self._timeout_secs, shell=True)
    ...
    def activate_workspace(self, ws_idx: int, wait: bool = False) -> None:
        p = subprocess.Popen([self._glazewm_path, "focus-workspace", str(ws_idx)], shell=True)

        if wait:
            p.wait()
    def next_workspace(self) -> None:
        try:
            subprocess.Popen([self._glazewm_path, "cycle-workspace", "next"], shell=True)
        except subprocess.SubprocessError:
            logging.exception("Failed to cycle glazewm workspace")

    def prev_workspace(self) -> None:
        try:
            subprocess.Popen([self._glazewm_path, "cycle-workspace", "prev"], shell=True)
        except subprocess.SubprocessError:
            logging.exception("Failed to cycle glazewm workspace")

    def toggle_focus_mouse(self) -> None:
        try:
            subprocess.Popen([self._glazewm_path, "toggle-focus-follows-mouse"], shell=True)
        except subprocess.SubprocessError:
            logging.exception("Failed to toggle focus-follows-mouse")

    def change_layout(self, layout: str) -> None:
        try:
            subprocess.Popen([self._glazewm_path, "change-layout", layout], shell=True)
        except subprocess.SubprocessError:
    ...
    def flip_layout(self, direction: str) -> None:
        try:
            subprocess.Popen(
                [self._glazewm_path, "flip-layout", direction],
    ...
    def toggle(self, toggle_type: str, wait: bool = False) -> None:
        try:
            p = subprocess.Popen([self._glazewm_path, f"toggle-{toggle_type}"], shell=True)
    ...
    def wait_until_subscribed_to_pipe(self, pipe_name: str):
        proc = subprocess.Popen(
            [self._glazewm_path, "subscribe", pipe_name],
            shell=True,
